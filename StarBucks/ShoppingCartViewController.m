...
{
    return ([self.arrCommodity count] + 2);
}
...
//        UIImageView *image = [[UIImageView alloc]initWithFrame:CGRectMake(50, 5, 60, 60)];
//        image.image = [UIImage imageNamed:[self.arrCommodity objectAtIndex:[indexPath row]]];
        
//        self.m_Cell = [[CellForShoppingCartViewController alloc]initWithNibName:@"CellForShoppingCart" bundle:[NSBundle mainBundle] data:[self.arrCommodity objectAtIndex:[indexPath row]]];
//        NSLog(@"%@",[self.arrCommodity objectAtIndex:[indexPath row]]);
//        self.m_Cell.nameCommodity.text = [self.arrCommodity objectAtIndex:[indexPath row]];
//        [cell addSubview:self.m_Cell.view];
